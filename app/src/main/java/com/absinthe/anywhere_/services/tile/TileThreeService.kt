...
class TileThreeService : TileService() {

    override fun onStartListening() {
        super.onStartListening()

        qsTile?.let {
            val label = SPUtils.getString(this, Const.PREF_TILE_THREE_LABEL)
            if (label.isNotEmpty()) {
                it.label = label
            }
            it.state = if (GlobalValues.tileThreeActive) Tile.STATE_ACTIVE else Tile.STATE_INACTIVE
    ...
    override fun onClick() {
        ...
        val intent = Intent(this, ShortcutsActivity::class.java).apply {
            ...
            putExtra(Const.INTENT_EXTRA_FROM_TILE, Const.PREF_TILE_THREE)
        }
